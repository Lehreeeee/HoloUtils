...
public class ModelEngineHook {
    ...
    public static void mountStatusDisplay(Entity targetEntity, TextDisplay display){
        ...
        LoggerUtil.debug(MessageFormat.format("Found {0} active models for this entity.", activeModels.size()));
        ...
        for(ActiveModel activeModel : activeModels){
            ...
            if (mountManager != null && mountManager.getSeats().containsKey("statuseffect")) {
                ...
                LoggerUtil.debug("Found statuseffect seat, mounting the display.");
        ...
        if(!foundSeat){
            LoggerUtil.warning("Unable to find a seat to mount status display for entity - " + targetEntity.getName()
